...
For each league with enough data:
  Sweep (MAX_DRAW_PROB, MIN_BET_PROB, MIN_GEM_SCORE)
...
def run() -> None:
    ...
    for league, total in league_counts.items():
        ...
        league_mask = covered & odds_valid & (leagues == league)
    ...
    # Console
    print("\n" + "=" * 110)
    print("PER-LEAGUE OPTIMAL GEM THRESHOLDS")
    print("=" * 110)
    print(f"{'League':>22s} | {'mxD':>5s} {'mBet':>5s} {'mGem':>5s} | "
    ...
    for league, t in rows_sorted:
        ...
        print(
            f"{league[:22]:>22s} | {t['max_draw_prob']:>5.2f} {t['min_bet_prob']:>5.2f} {t['min_gem_score']:>5.2f} | "
    ...
    print("-" * 110)
    print(f"{'TOTAL':>22s} | {' '*17} | {total_n:>4d} {' '*38} {total_n/HISTORY_YEARS:>5.1f} {total_yr:>+8.2f}")
