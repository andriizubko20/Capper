...
OUT        = '/app/BETA/best_models_analysis_v2.xlsx'
FLAT_STAKE = 50.0
...
MODELS = {
    ...
    'Bundesliga': [
        ...
        'away[2.5,3.5) elo<=-150','away[2.2,2.8) xg>=1.5 mkt>=0.4',
        'away[2.2,2.8) mkt>=0.4','away[2.2,2.8) elo<=-75 mkt>=0.35',
        'away[2.0,2.5) mkt>=0.4','away[1.7,2.0) xg>=1.0 form>=2.2',
    ],
    'Serie A': [
        ...
        'away[1.8,2.2) xg>=1.5 form>=2.2','away[1.7,2.0) xg>=1.5 form>=2.2',
    ],
    'La Liga': [
        'home[2.2,2.8) xg>=1.0 form>=1.5','home[2.0,2.5) xg>=1.0 form>=1.5',
        'home[1.8,2.2) xg>=1.2 form>=1.5','home[1.8,2.2) xg>=1.0 mkt>=0.5',
        'home[1.8,2.2) xg>=1.0 form>=1.5 mkt>=0.5','home[1.8,2.2) xg>=1.0 elo>=30 mkt>=0.5',
        'home[1.55,1.8) xg>=1.8','away[2.5,3.5) xg>=1.8 form>=1.5',
        'away[2.5,3.5) elo<=-75 form>=2.2','away[2.2,2.8) xg>=1.2 mkt>=0.4',
    ...
    'Serie B': [
        ...
        'home[1.8,2.2) xg>=1.0 form>=2.2','home[1.7,2.0) xg>=1.0 form>=2.2',
        'home[1.55,1.8) xg>=1.8','home[1.55,1.8) xg>=1.5 elo>=75 form>=2.2',
        'home[1.55,1.8) xg>=1.2 elo>=150','home[1.55,1.8) xg>=1.5',
...
matches_raw['home_name'] = matches_raw['home_team_id'].map(teams_map)
matches_raw['away_name'] = matches_raw['away_team_id'].map(teams_map)
name_map = matches_raw.set_index('match_id')[['home_name','away_name']]

# merge names into df (match_id col)
if 'match_id' in df.columns:
    df = df.join(name_map, on='match_id')
else:
    df['home_name'] = df['home_team_id'].map(teams_map) if 'home_team_id' in df.columns else '?'
    df['away_name'] = df['away_team_id'].map(teams_map) if 'away_team_id' in df.columns else '?'
...
for league, niches in MODELS.items():
    ...
    for niche_str in niches:
        ...
        if len(bets)==0:
            niche_data.append({'niche':niche_str,'ids':set()}); continue
        monthly={}
        ...
        ins_i=[(m,v) for m,v in monthly.items() if m<oos_p]
        oos_i=[(m,v) for m,v in monthly.items() if m>=oos_p]
        ins_pl=sum(v['pl'] for _,v in ins_i); ins_n=sum(v['n'] for _,v in ins_i); ins_w=sum(v['w'] for _,v in ins_i)
        oos_pl=sum(v['pl'] for _,v in oos_i); oos_n=sum(v['n'] for _,v in oos_i); oos_w=sum(v['w'] for _,v in oos_i)
        niche_data.append({
            ...
            'roi_pct':total_pl/(total_n*FLAT_STAKE)*100,'wr_pct':total_w/total_n*100,
            'ins_roi':ins_pl/(ins_n*FLAT_STAKE)*100 if ins_n else 0,'ins_wr':ins_w/ins_n*100 if ins_n else 0,
            'oos_roi':oos_pl/(oos_n*FLAT_STAKE)*100 if oos_n else 0,'oos_wr':oos_w/oos_n*100 if oos_n else 0,
            'oos_n':oos_n,'ins_n':ins_n,'oc':oc,'wc':wc,
            'total_pl_val':total_pl,'oos_pl_val':oos_pl,'ins_pl_val':ins_pl,
        })
    ...
    for r in niche_data:
        ...
        mv=[r['niche'],r['side'],r['total_n'],round(r['total_pl'],0),round(roi,1),round(r['wr_pct'],1),
            round(ins_roi,1),round(r['ins_wr'],1),round(oos_roi,1),round(r['oos_wr'],1),int(r['oos_n'])]
        for ci,val in enumerate(mv,1):
            c=ws.cell(row_idx,ci,val); c.border=border; c.alignment=Alignment(horizontal='center')
    ...
    if len(valid)>1:
        row_idx+=1
        c=ws.cell(row_idx,1,'OVERLAP MATRIX (% of row matched by column)')
        ...
        row_idx+=1
        ws.cell(row_idx,1,'↓ row  →  col').font=Font(bold=True,size=8)
        for ci,r in enumerate(valid,2):
            c=ws.cell(row_idx,ci,r['niche'][:22]); c.fill=fill_oos_h
            c.font=Font(color='FFFFFF',bold=True,size=7); c.alignment=Alignment(horizontal='center',wrap_text=True)
        ws.row_dimensions[row_idx].height=40; row_idx+=1
        for ra in valid:
            ...
            for ci,rb in enumerate(valid,2):
                if ra is rb:
                    ...
                    ovlp=len(ra['ids']&rb['ids'])/len(ra['ids'])*100 if ra['ids'] else 0
                    c=ws.cell(row_idx,ci,round(ovlp,0))
                    c.number_format='0"%"'; c.alignment=Alignment(horizontal='center'); c.font=Font(size=8)
                    c.fill=fill_neg if ovlp>=70 else (fill_ovlp if ovlp>=40 else fill_p)
                c.border=border
            row_idx+=1
    ...
    # League P&L totals
    valid_nd=[r for r in niche_data if r.get('total_n')]
    lg_total_pl=sum(r['total_pl_val'] for r in valid_nd)
    lg_oos_pl  =sum(r['oos_pl_val'] for r in valid_nd)
    lg_ins_pl  =sum(r['ins_pl_val'] for r in valid_nd)
    lg_total_n =sum(r['total_n'] for r in valid_nd)
    lg_oos_n   =sum(r['oos_n'] for r in valid_nd)
    league_summary.append({
        'League':league,'Models':len(valid_nd),
        'Total Bets':lg_total_n,'Total P&L':round(lg_total_pl,0),
        'Overall ROI%':round(lg_total_pl/(lg_total_n*FLAT_STAKE)*100,1) if lg_total_n else 0,
        'IS P&L':round(lg_ins_pl,0),'OOS Bets':lg_oos_n,
        'OOS P&L':round(lg_oos_pl,0),
        'OOS ROI%':round(lg_oos_pl/(lg_oos_n*FLAT_STAKE)*100,1) if lg_oos_n else 0,
    })

    # Collect matches
    for r in valid_nd:
        bets=r['bets']; wc=r['wc']; oc=r['oc']
        for _,bet in bets.sort_values('date').iterrows():
            won=bool(bet[wc]); odds=float(bet[oc])
            pl=FLAT_STAKE*(odds-1) if won else -FLAT_STAKE
            hname=bet.get('home_name','') or str(bet.get('home_team_id',''))
            aname=bet.get('away_name','') or str(bet.get('away_team_id',''))
            all_match_rows.append({
                'Date':  pd.Timestamp(bet['date']).date(),
                'League':league,
                'Match': f"{hname} vs {aname}",
                'Bet':   r['side'],
                'Team':  hname if r['side']=='home' else aname,
                'Odds':  round(odds,2),
                'Model': r['niche'],
                'Result':bet.get('result',''),
                'Won':   '✓' if won else '✗',
                'P&L':   round(pl,1),
                'Period':'OOS' if bet['ym']>=oos_p else 'IS',
            })

# ── Match List sheet ──────────────────────────────────────────────────────────
print('Building match list...')
match_df=pd.DataFrame(all_match_rows).sort_values(['Date','League','Model']).reset_index(drop=True)

ws_ml=wb.create_sheet('Match List',0)
ml_cols=['Date','League','Match','Bet','Team','Odds','Model','Result','Won','P&L','Period']
...
for ri,(_,row) in enumerate(match_df.iterrows(),2):
    ...
    for ci,col in enumerate(ml_cols,1):
        ...
        if ci in (2,3,5,7): c.alignment=Alignment(horizontal='left')
        if ci==9:
            c.fill=fill_ps if val=='✓' else fill_neg
            c.font=Font(bold=True)
        if ci==10:
            c.number_format='+0.0;-0.0;0'
            c.fill=fill_p if float(row['P&L'])>0 else fill_neg
        if ci==11:
            c.fill=fill_oos_h if is_oos else PatternFill()
            if is_oos: c.font=Font(color='FFFFFF',bold=True)

ml_widths=[12,16,32,6,18,7,42,7,5,8,6]
for i,w in enumerate(ml_widths,1): ws_ml.column_dimensions[get_column_letter(i)].width=w
ws_ml.freeze_panes='A2'
ws_ml.auto_filter.ref=f'A1:{get_column_letter(len(ml_cols))}1'

# ── League P&L Summary ────────────────────────────────────────────────────────
ws_lg=wb.create_sheet('League P&L',0)
lg_cols=['League','Models','Total Bets','Total P&L ($)','Overall ROI%','IS P&L ($)','OOS Bets','OOS P&L ($)','OOS ROI%']
...
for ri,r in enumerate(league_summary,2):
    vals=[r['League'],r['Models'],r['Total Bets'],r['Total P&L'],r['Overall ROI%'],
          r['IS P&L'],r['OOS Bets'],r['OOS P&L'],r['OOS ROI%']]
    for ci,val in enumerate(vals,1):
        c=ws_lg.cell(ri,ci,val); c.border=border; c.alignment=Alignment(horizontal='center')
        if ci==1: c.alignment=Alignment(horizontal='left')
        if ci in (4,6,8):
            c.number_format='$#,##0'
            c.fill=fill_ps if val>=500 else (fill_p if val>0 else fill_neg)
        if ci in (5,9):
            c.number_format='+0.0;-0.0;0'
            c.fill=fill_ps if val>=30 else (fill_p if val>0 else fill_neg)
            c.font=Font(bold=True)

# Totals row
total_row=['TOTAL',
    sum(r['Models'] for r in league_summary),
    sum(r['Total Bets'] for r in league_summary),
    sum(r['Total P&L'] for r in league_summary),
    0,
    sum(r['IS P&L'] for r in league_summary),
    sum(r['OOS Bets'] for r in league_summary),
    sum(r['OOS P&L'] for r in league_summary),
    0,
]
total_bets=sum(r['Total Bets'] for r in league_summary)
oos_bets=sum(r['OOS Bets'] for r in league_summary)
total_row[4]=round(total_row[3]/(total_bets*FLAT_STAKE)*100,1) if total_bets else 0
total_row[8]=round(total_row[7]/(oos_bets*FLAT_STAKE)*100,1) if oos_bets else 0
ri=len(league_summary)+2
for ci,val in enumerate(total_row,1):
    c=ws_lg.cell(ri,ci,val); c.border=border
    c.font=Font(bold=True,size=11); c.alignment=Alignment(horizontal='center')
    c.fill=fill_lg
    if ci in (4,6,8): c.number_format='$#,##0'
    if ci in (5,9): c.number_format='+0.0;-0.0;0'

lg_widths=[20,8,10,13,12,13,10,13,12]
...
for league,niches in MODELS.items():
    for niche_str in niches:
        side,lo,hi,xg,elo,fm,mk=parse_niche(niche_str)
        wc='home_win' if side=='home' else 'away_win'; oc='home_odds_val' if side=='home' else 'away_odds_val'
        bets=apply_mask(df,side,lo,hi,xg,elo,fm,mk,league)
        ...
        all_stats.append({
            ...
            'ROI%':total_pl/(total_n*FLAT_STAKE)*100,'WR%':total_w/total_n*100,
            ...
            'OOS WR%':oos[wc].mean()*100 if len(oos) else 0,'OOS n':len(oos),
        })
...
print(f'Saved: {OUT}')
print()
print('=== League P&L Summary ===')
for r in league_summary:
    print(f"  {r['League']:<22s}  Total P&L=${r['Total P&L']:>7.0f}  OOS P&L=${r['OOS P&L']:>7.0f}  OOS ROI={r['OOS ROI%']:>+6.1f}%")
total_pl=sum(r['Total P&L'] for r in league_summary)
total_oos=sum(r['OOS P&L'] for r in league_summary)
print(f"  {'TOTAL':<22s}  Total P&L=${total_pl:>7.0f}  OOS P&L=${total_oos:>7.0f}")
print('✅ Done!')
