...
V5 — Portfolio Single Bankroll.

Всі ніші — один спільний банкрол $1000.
Кожна ніша має свій pre-filter, свою міні-модель і свій Kelly cap.
Ставки з усіх ніш йдуть в одну хронологічну чергу.

Ніші (підтверджені OOS walk-forward v4):
  A: Away [1.55-1.80] + elo<=-30              → cap=5%
  B: Home [1.80-2.20] + xg>=1.5 + mkt>=0.45  → cap=7%
  C: Home [1.30-1.55] + xg>=1.5 + form>=1.8  → cap=6%
  D: Home [1.55-1.80] + xg>=1.5 + mkt>=0.50  → cap=6%
"""
import sys, os
...
def run_portfolio(df, feat_cols, verbose=True):
    ...
    all_bets     = []   # всі ставки з усіх ніш
    period_stats = []
    ...
    for test_start, test_end in splits:
        ...
        used_matches = set()  # уникаємо дублів (один матч — одна ставка)
        ...
        for nid, cfg in NICHES.items():
            ...
            # Прибираємо матчі, які вже є в іншій ніші цього периоду
            ...
            cal_size = max(25, int(len(train_n) * 0.20))
        ...
        if verbose:
            ...
            for nid, v in period_info['niches'].items():
                ...
                niche_parts.append(f"{nid}={v['n']}({val})")
            niche_str = '  '.join(niche_parts)
            print(f"  {test_start.strftime('%Y-%m')}–{test_end.strftime('%Y-%m')}: "
                  f"total={n_total:3d} win={period_info['wr_total']:.0f}%  |  {niche_str}")

    # ── Єдиний банкрол — хронологічна черга ────────────────────────────────────
...
def print_report(r):
    ...
    test_months = len(r['period_stats']) * STEP_MONTHS
    bpy = r['n_bets'] / (test_months / 12) if test_months > 0 else 0
    proj_2yr = INITIAL_BANKROLL * (x ** (24 / test_months)) if test_months > 0 else 0

    print(f"\n{'='*60}")
    print(f"  BETA v5 — Portfolio Single Bankroll")
    print(f"{'='*60}")
    ...
    print(f"  2yr proj:   ${proj_2yr:.0f}")

    print(f"\n  По нішах:")
    print(f"  {'Ніша':<45} {'Ставок':>7} {'WR':>6} {'Profit':>9}")
    print(f"  {'-'*70}")
    for nid, cfg in NICHES.items():
        ns = r['niche_stats'][nid]
        n  = ns['wins'] + ns['losses']
        wr = ns['wins'] / n * 100 if n > 0 else 0
        print(f"  {cfg['label']:<45} {n:>7} {wr:>5.1f}% ${ns['profit']:>+8.1f}")
    ...
    for p in r['period_stats']:
        ...
        print(f"    {p['test_start'].strftime('%Y-%m')} | "
              f"total={p['n_total']:3d} win={p['wr_total']:5.1f}%  |  {niche_str}")
